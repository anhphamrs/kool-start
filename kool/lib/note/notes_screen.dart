...
class _NotesScreenState extends State<NotesScreen> {
  ...
  Future<String?> getCategoryName() async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    categoryName = prefs.getString('topic');
    return categoryName;
  }

  Future<String?> getNoteName() async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    notName = prefs.getString('noteName');
    return notName;
  }
  ...
  Future<void> saveNote(String content) async {
    try {
      ...
      notName = await getNoteName();
      userId = await getUserId();
      final response = await http.post(
        Uri.parse('http://$serverIp:8080/home/$userId/note/savenote'),
              ...
              children: [
                IconButton(
                    onPressed: () async {
                      ...
                      prefs.setString(
                          'noteContent', _controller.document.toPlainText());
                      await saveNote(_controller.document.toPlainText());
                    },
