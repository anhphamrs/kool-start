...
''',
  ...
  Future<void> saveCategory(String topic) async {
    try {
      userId = await getUserId();
      final response = await http.post(
        Uri.parse('http://$serverIp:8080/home/$userId/note/savecategory'),
      ...
      if (response.body == null) {
        final parejson = jsonDecode(response.body);
        final SharedPreferences prefs = await SharedPreferences.getInstance();
        prefs.setString('categoryName', parejson['topic']);
      }
    } catch (e) {
      print(e);
    }
  ...
  // Future<void> getCategory(String topic) async {
  //   try {
  //     userId = await getUserId();
